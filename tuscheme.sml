...
fun find (name, []) = raise NotFound name
  | find (name, (x, v) :: tail) =
      if name = x then v else find (name, tail)
(* support for names and environments 305c *)
fun isbound (name, []) = false
  | isbound (name, (x, v) :: tail) =
      name = x orelse isbound (name, tail)
(* support for names and environments 305d *)
fun bind (name, v, rho) = (name, v) :: rho
(* support for names and environments 305e *)
exception BindListLength
fun bindList (x :: vars, v :: vals, rho) =
      bindList (vars, vals, bind (x, v, rho))
  | bindList ([], [], rho) = rho
...
fun duplicatename [] = NONE
  | duplicatename (x :: xs) =
      if List.exists (fn x' => x' = x) xs then SOME x else duplicatename xs
(* type declarations for consistency checking *)
...
fun fnvHash s =
  let
    val offset_basis = 0wx011C9DC5 : Word.word (* trim the high bit *)
    val fnv_prime = 0w16777619 : Word.word
    fun update (c, hash) =
      Word.xorb (hash, Word.fromInt (ord c)) * fnv_prime
    fun int w =
      Word.toIntX w
      handle Overflow => Word.toInt (Word.andb (w, 0wxffffff))
  in
    int (foldl update offset_basis (explode s))
  end
(* type declarations for consistency checking *)
...
fun withXprinter xp f x =
  let
    ...
    val () = xprinter := xp
  in
    tryFinally f x (fn () => xprinter := oxp)
  end
(* type declarations for consistency checking *)
...
fun printUTF8 code =
  let
    val w = Word.fromInt code
    val (&, >>) = (Word.andb, Word.>>)
    infix 6 & >>
    val _ =
      if (w & 0wx1fffff) <> w then
        raise RuntimeError
          (intString code ^ " does not represent a Unicode code point")
      else
        ()
  ...
  in
    if w > 0wxffff then
      ...
      app printbyte [prefix 0wxc0 (w >> 0w6), prefix 0wx80 ((w) & 0wx3f)]
    else
      printbyte w
  end
...
fun stripNumericSuffix s =
  let
    fun stripPrefix [] = s (* don't let things get empty *)
      | stripPrefix (#"-" :: []) = s
      | stripPrefix (#"-" :: cs) =
          implode (reverse cs)
      | stripPrefix (c :: cs) =
          if Char.isDigit c then stripPrefix cs else implode (reverse (c :: cs))
  in
    stripPrefix (reverse (explode s))
  end
...
local
  ...
  val optionTokens = getOpt
    (Option.map split (OS.Process.getEnv "BPCOPTIONS"), [])
in
...
fun joinC (C xs) =
  C (List.concat (map elemsC xs))
fun mapC f (C xs) =
  C (map f xs)
fun filterC p (C xs) =
  C (List.filter p xs)
fun mapC2 f (xc, yc) =
  joinC (mapC (fn x => mapC (fn y => f (x, y)) yc) xc)
(* type declarations for consistency checking *)
...
val _ =
  op mapC2 : ('a * 'b -> 'c) -> ('a collection * 'b collection -> 'c collection)
(* suspensions S228a *)
datatype 'a action =
  PENDING of unit -> 'a
| PRODUCED of 'a
...
fun demand cell =
  case !cell of
    PENDING f => let val result = f () in (cell := PRODUCED result; result) end
  | PRODUCED v => v
(* type declarations for consistency checking *)
...
(* streams S230c *)
val naturals = streamOfUnfold (fn n => SOME (n, n + 1)) 0 (* 0 to infinity *)
(* type declarations for consistency checking *)
...
fun streamFilter p xs =
  delayedStream (fn () =>
    case streamGet xs of
      NONE => EOS
    | SOME (x, xs) => if p x then x ::: streamFilter p xs else streamFilter p xs)
(* type declarations for consistency checking *)
...
fun streamZip (xs, ys) =
  delayedStream (fn () =>
    case (streamGet xs, streamGet ys) of
      (SOME (x, xs), SOME (y, ys)) => (x, y) ::: streamZip (xs, ys)
    | _ => EOS)
(* streams S232a *)
fun streamConcat xss =
  let
    fun get (xs, xss) =
      case streamGet xs of
        SOME (x, xs) => SOME (x, (xs, xss))
      | NONE =>
          case streamGet xss of
            SOME (xs, xss) => get (xs, xss)
          | NONE => NONE
  in
    streamOfUnfold get (EOS, xss)
  end
(* type declarations for consistency checking *)
...
(* streams S232b *)
fun streamConcatMap f xs =
  streamConcat (streamMap f xs)
(* type declarations for consistency checking *)
val _ = op streamConcatMap : ('a -> 'b stream) -> 'a stream -> 'b stream
(* streams S232c *)
infix 5 @@@
fun xs @@@ xs' =
  streamConcat (streamOfList [xs, xs'])
(* type declarations for consistency checking *)
...
fun sat p tx xs =
  case tx xs of
    answer as SOME (OK y, xs) => if p y then answer else NONE
  | answer => answer
(* type declarations for consistency checking *)
...
(* type declarations for consistency checking *)
val _ =
  op<*>! : ('a, 'b -> 'c error) xformer * ('a, 'b) xformer -> ('a, 'c) xformer
val _ = op<$>! : ('b -> 'c error) * ('a, 'b) xformer -> ('a, 'c) xformer
(* support for source-code locations and located streams S233b *)
type srcloc = string * int
fun srclocString (source, line) = source ^ ", line " ^ intString line
(* support for source-code locations and located streams S233c *)
datatype error_format =
  WITH_LOCATIONS
| WITHOUT_LOCATIONS
...
fun synerrormsg (source, line) strings =
  if
    !toplevel_error_format = WITHOUT_LOCATIONS andalso source = "standard input"
  then
    concat ("syntax error: " :: strings)
  else
    concat
      ("syntax error in " :: srclocString (source, line) :: ": " :: strings)
...
fun warnAt (source, line) strings =
  ( app eprint
      (if
       ...
       then "warning: " :: strings
       else "warning in " :: srclocString (source, line) :: ": " :: strings)
  ; eprint "\n"
  )
...
fun atLoc loc f a =
  f a
  handle
    e as RuntimeError _ => raise Located (loc, e)
    ...
    val splice_in = Substring.full
      (case maybeLoc of
       ...
       | SOME (loc as (file, line)) =>
           ...
           then ""
           else " in " ^ srclocString loc)
  in
    if Substring.size atloc = 0 then (* <at loc> is not present *) s
    else Substring.concat [prefix, splice_in, suffix]
  end
fun fillAtLoc (s, loc) =
  fillComplaintTemplate (s, SOME loc)
...
fun locatedStream (streamname, inputs) =
  let
    val locations = streamZip
      (streamRepeat streamname, streamDrop (1, naturals))
  in
    streamZip (locations, inputs)
  end
(* type declarations for consistency checking *)
...
in
  fun eol xs =
    (asEol <$>? one) xs
  fun inline xs =
    (asInline <$>? many eol *> one) xs
  fun srcloc xs =
    rewind (fst <$> inline) xs
end
(* type declarations for consistency checking *)
...
fun intFromChars (#"-" :: cs) = intFromChars cs >>=+ Int.~
  | intFromChars cs =
...
fun intToken isDelim = intFromChars <$>! intChars isDelim
(* type declarations for consistency checking *)
val _ = op intToken : (char -> bool) -> int lexer
(* support for lexical analysis S256d *)
datatype bracket_shape =
  ROUND
| SQUARE
| CURLY
...
fun bracketLexer pretoken =
  LEFT ROUND <$ eqx #"(" one <|> LEFT SQUARE <$ eqx #"[" one
  <|> LEFT CURLY <$ eqx #"{" one <|> RIGHT ROUND <$ eqx #")" one
  <|> RIGHT SQUARE <$ eqx #"]" one <|> RIGHT CURLY <$ eqx #"}" one
  <|> PRETOKEN <$> pretoken
(* type declarations for consistency checking *)
...
fun asAscii p =
  let
    fun good c = Char.isPrint c andalso Char.isAscii c
    fun warn (loc, s) =
      case List.find (not o good) (explode s) of
        NONE => OK s
      | SOME c =>
          let
...
infix 0 <?>
fun p <?> what =
  p <|> synerrorAt ("expected " ^ what) <$>! srcloc
(* type declarations for consistency checking *)
  ...
  fn tokens =>
    (case p tokens of
       SOME (OK _, unread) =>
         let
           val outcome =
             case peek srcloc tokens of
               SOME loc => synerrorAt msg loc
             | NONE => ERROR msg
...
fun rejectReserved reserved x =
  if member x reserved then
    ERROR
      ("syntax error: " ^ x ^ " is a reserved word and "
...
fun left tokens =
  ((fn (loc, LEFT s) => SOME (loc, s) | _ => NONE) <$>? inline) tokens
fun right tokens =
  ((fn (loc, RIGHT s) => SOME (loc, s) | _ => NONE) <$>? inline) tokens
fun pretoken stream =
  ((fn PRETOKEN t => SOME t | _ => NONE) <$>? token) stream
(* type declarations for consistency checking *)
...
fun notCurly (_, CURLY) = false
  | notCurly _ = true
fun leftCurly tokens =
  sat (not o notCurly) left tokens
(* transformers for interchangeable brackets S262c *)
(* definition of function [[errorAtEnd]] S261b *)
infix 4 errorAtEnd
fun p errorAtEnd mkMsg =
  fn tokens =>
    (case (p <* rewind right) tokens of
       SOME (OK s, unread) =>
         let
           val outcome =
             case peek srcloc tokens of
               SOME loc => synerrorAt ((concat o mkMsg) s) loc
...
(* type declarations for consistency checking *)
val _ =
  op errorAtEnd : ('t plus_brackets, 'a) polyparser * ('a -> string list)
...
fun matchingRight tokens =
  ...
  in
    (FOUND_RIGHT <$> right <|> scanToClose) tokens
  end
(* type declarations for consistency checking *)
...
fun matchBrackets _ (loc, left) a (FOUND_RIGHT (loc', right)) =
      if left = right then
        ...
        synerrorAt
          (rightString right ^ " does not match " ^ leftString left
...
(* type declarations for consistency checking *)
val _ =
  op matchBrackets : string
                     -> bracket_shape located
                     -> 'a
                     -> right_result
                     -> 'a error
...
(* type declarations for consistency checking *)
val _ =
  op bracketKeyword : ('t, 'keyword) pb_parser * string * ('t, 'a) pb_parser
                      -> ('t, 'a) pb_parser
(* transformers for interchangeable brackets S264b *)
fun usageParser keyword =
  let
    val left = eqx #"(" one <|> eqx #"[" one
    val getkeyword = left *> (implode <$> many1 (sat (not o isDelim) one))
  in
    fn (usage, p) =>
      case getkeyword (streamOfList (explode usage)) of
        SOME (OK k, _) => bracketKeyword (keyword k, usage, p)
      | _ => raise InternalError ("malformed usage string: " ^ usage)
  end
(* type declarations for consistency checking *)
val _ =
  op usageParser : (string -> ('t, string) pb_parser)
                   -> string * ('t, 'a) pb_parser
                   -> ('t, 'a) pb_parser
(* code used to debug parsers S265c *)
fun safeTokens stream =
  let
    fun tokens (seenEol, seenSuspended) =
      let
        fun get (EOL _ ::: ts) =
              if seenSuspended then [] else tokens (true, false) ts
          | get (INLINE (_, t) ::: ts) = t :: get ts
          | get EOS = []
          | get (SUSPENDED (ref (PRODUCED ts))) = get ts
          | get (SUSPENDED s) =
              if seenEol then [] else tokens (false, true) (demand s)
      in
        get
      end
  in
    tokens (false, false) stream
  end
(* type declarations for consistency checking *)
...
fun showErrorInput asString p tokens =
  case p tokens of
    result as SOME (ERROR msg, rest) =>
      if String.isSubstring " [input: " msg then
        result
      else
        SOME
          ( ERROR
              (msg ^ " [input: " ^ spaceSep (map asString (safeTokens tokens))
               ^ "]")
          , rest
          )
  | result => result
...
fun echoTagStream lines =
  let
    fun echoIfTagged line =
      if (String.substring (line, 0, 2) = ";#" handle _ => false) then
        print line
      else
        ()
  in
    postStream (lines, echoIfTagged)
  end
(* type declarations for consistency checking *)
...
fun stripAndReportErrors xs =
  let
    fun next xs =
      case streamGet xs of
        SOME (ERROR msg, xs) => (eprintln msg; next xs)
      | SOME (OK x, xs) => SOME (x, xs)
      | NONE => NONE
  in
    streamOfUnfold next xs
  end
(* type declarations for consistency checking *)
...
fun parseWithErrors parser =
  let
    fun adjust (SOME (ERROR msg, tokens)) =
          SOME (ERROR msg, drainLine tokens)
      | adjust other = other
  in
    streamOfUnfold (adjust o parser)
  end
(* type declarations for consistency checking *)
val _ =
  op parseWithErrors : ('t, 'a) polyparser
                       -> 't located eol_marked stream
                       -> 'a error stream
...
fun ('t, 'a) interactiveParsedStream (lexer, parser) (name, lines, prompts) =
  let
    ...
    fun lexAndDecorate (loc, line) =
      let
        val tokens = postStream (lexLineWith lexer line, setPrompt ps2)
      in
        streamMap INLINE (streamZip (streamRepeat loc, tokens))
    ...
    (* type declarations for consistency checking *)
    val _ =
      op interactiveParsedStream : 't lexer * ('t, 'a) polyparser
                                   -> string * line stream * prompts
                                   -> 'a stream
...
fun ('t, 'a) finiteStreamOfLine fail (lexer, parser) line =
  let
    val lines = streamOfList [line] @@@ streamOfEffects fail
    fun lexAndDecorate (loc, line) =
      let
        val tokens = lexLineWith lexer line
      in
        streamMap INLINE (streamZip (streamRepeat loc, tokens))
        @@@ streamOfList [EOL (snd loc)]
      end
...
val _ =
  finiteStreamOfLine : (unit -> string option)
                       -> 't lexer * ('t, 'a) polyparser
                       -> line
                       -> 'a stream
...
in
  ...
  fun withFuel n f x =
    let
      ...
      val _ = evalFuel := n
    in
      (f x before evalFuel := old)
      handle e => (evalFuel := old; raise e)
    end
  ...
  fun applyWithLimits f =
    if !recursionLimit <= 0 then
      ...
      let
        val _ = recursionLimit := !recursionLimit - 1
        val _ = evalFuel := !evalFuel - 1
      in
        fn arg => f arg before (recursionLimit := !recursionLimit + 1)
      end
  fun resetComputationLimits () =
    (recursionLimit := defaultRecursionLimit; evalFuel := !defaultEvalFuel)
end
...
datatype tyex =
  TYCON of name (* type constructor *)
...
(* function to construct the binary tree type (see Recitation 07) *)
fun btreetype ty =
  CONAPP (TYCON "btree", [ty])
(* functions to construct the pair and sum types (see bonus problem) *)
fun pairtype (a, b) =
  CONAPP (TYCON "pair", [a, b])
fun sumtype (a, b) =
  CONAPP (TYCON "sum", [a, b])
(* type declarations for consistency checking *)
...
val _ = op pairtype : tyex * tyex -> tyex
val _ = op sumtype : tyex * tyex -> tyex
...
fun freetyvars t =
  let
    fun free (TYVAR v, ftvs) = insert (v, ftvs)
      | free (TYCON _, ftvs) = ftvs
      | free (CONAPP (ty, tys), ftvs) =
          foldl free (free (ty, ftvs)) tys
      | free (FUNTY (tys, ty), ftvs) =
          foldl free (free (ty, ftvs)) tys
      | free (FORALL (alphas, tau), ftvs) =
          union (diff (free (tau, emptyset), alphas), ftvs)
  in
    reverse (free (t, emptyset))
  end
(* type declarations for consistency checking *)
...
fun freshName (alpha, avoid) =
  let
    val basename = stripNumericSuffix alpha
    val candidates = streamMap (fn n => basename ^ "-" ^ intString n) naturals
    fun ok beta =
      not (member beta avoid)
  in
    case streamGet (streamFilter ok candidates) of
      SOME (beta, _) => beta
  ...
  let (* definition of internal function [[kind]] 378c *)
    fun kind (TYVAR a) =
      ...
      | kind (TYCON c) =
          (find (c, Delta)
           handle NotFound _ =>
             raise TypeError ("unknown type constructor " ^ c))
      (* definition of internal function [[kind]] 379a *)
      | kind (FUNTY (args, result)) =
          let
            fun badKind tau =
              not (eqKind (kind tau, TYPE))
          in
            if badKind result then
          ...
          (case kind tau of
             ARROW (formal_kinds, result_kind) =>
               if eqKinds (formal_kinds, map kind actuals) then
                 ...
                 raise TypeError
                   ("type constructor " ^ typeString tau
                    ^ " applied to the wrong arguments")
           | TYPE =>
               raise TypeError
                 ("tried to apply type " ^ typeString tau
                  ^ " as type constructor"))
...
fun asType (ty, Delta) =
  case kindof (ty, Delta) of
    TYPE => ty
  | ARROW _ =>
      raise TypeError
        ("used type constructor `" ^ typeString ty ^ "' as a type")
(* type declarations for consistency checking *)
...
fun valueString (SYM v) = v
  ...
  | valueString (PAIR (car, cdr)) =
      let
        fun tail (PAIR (car, cdr)) =
              " " ^ valueString car ^ tail cdr
          | tail NIL = ")"
          | tail v =
              " . " ^ valueString v ^ ")"
        (* type declarations for consistency checking *)
...
fun expString e =
  let
    ...
    fun formal (x, tau) =
      sqSpace [x, ":", typeString tau]
    fun withBindings (keyword, bs, e) =
      bracket (spaceSep [keyword, bindings bs, expString e])
    and bindings bs =
      bracket (spaceSep (map binding bs))
    and binding (x, e) =
      bracket (x ^ " " ^ expString e)

    fun tybinding ((x, ty), e) =
      bracketSpace [formal (x, ty), expString e]
    and tybindings bs =
      bracket (spaceSep (map tybinding bs))
  ...
  in
    ...
    | DEFINE (f, rtau, (formals, body)) =>
        bracketSpace
          ...
          , typeString rtau
          , f
          , bracketSpace (map formal formals)
          , expString body
          ]
  end
...
fun cycleThrough xs =
  let
    val remaining = ref xs
    fun next () =
      case !remaining of
        [] => (remaining := xs; next ())
      | x :: xs => (remaining := xs; x)
  in
    if null xs then raise InternalError "empty list given to cycleThrough"
    else next
  end
val unspecified = cycleThrough
  [ BOOLV true
  , NUM 39
  , SYM "this value is unspecified"
  , NIL
...
fun tysubst (tau, varenv) =
  let
    ...
    fun subst (TYVAR a) =
          (find (a, varenv) handle NotFound _ => TYVAR a)
      | subst (TYCON c) = (TYCON c)
      | subst (CONAPP (tau, taus)) =
          CONAPP (subst tau, map subst taus)
      | subst (FUNTY (taus, tau)) =
          FUNTY (map subst taus, subst tau)
      | subst (FORALL (alphas, tau)) =

          (* use [[varenv]] to substitute in [[tau]]; don't capture or substitute for any [[alphas]] 375b *)
          let
            val free = freetyvars (FORALL (alphas, tau))
            val new_taus = map (subst o TYVAR) free
            val potential_captures = foldl union emptyset
              (map freetyvars new_taus)
            val actual_captures = inter (potential_captures, alphas)
          in
            if null actual_captures then
...
fun instantiate (FORALL (formals, tau), actuals, Delta) =
      (case List.find (fn t => not (eqKind (kindof (t, Delta), TYPE))) actuals of
         SOME t =>
           raise TypeError
             ("instantiated at type constructor `" ^ typeString t
              ^ "', which is not a type")
       | NONE =>
  ...
  | instantiate (tau, _, _) =
      raise TypeError
        ("tried to instantiate term " ^ "of non-quantified type "
         ^ typeString tau)
(* type declarations for consistency checking *)
...
val infiniteTyvars = streamMap (fn n => "'b-" ^ intString n) naturals
(* type declarations for consistency checking *)
val _ = op naturals : int stream
    ...
    fun ty (LITERAL (NUM n)) = inttype
      ...
      | ty (APPLY (f, actuals)) = 
          let
             ...
             val a_types = map (fn (e) => ty e) actuals

             (* function to ensure argument types and actual types are equal *)
          ...
          in
             ...
             case typeof(f,Delta,Gamma) of
                FUNTY (args, ret) => 
                   (* check that the arg types and a_types are same *)
                   if (check_eq (args, a_types)) then
                      ret
                   else
...
local
  ...
  fun noneIfLineEnds chars =
    case streamGet chars of
      NONE => NONE (* end of line *)
    | SOME (#";", cs) => NONE (* comment *)
    | SOME (c, cs) =>
        let
          val msg =
            "invalid initial character in `" ^ implode (c :: listOfStream cs)
            ^ "'"
        in
          SOME (ERROR msg, EOS)
        end
...
in
  val schemeToken =
    ...
    *>
    bracketLexer
      (QUOTE <$ eqx #"'" one <|> INT <$> intToken isDelim
...
val pretoken =
  (fn (PRETOKEN t) => SOME t | _ => NONE) <$>? token : pretoken parser
...
fun keyword words =
  let
    fun isKeyword s =
      List.exists (fn s' => s = s') words
  in
    sat isKeyword namelike
  end
...
val _ =
  op bindingsOf : string -> 'x parser -> 'e parser -> ('x * 'e) list parser
(* type declarations for consistency checking *)
val _ =
  op distinctBsIn : (name * 'e) list parser -> string -> (name * 'e) list parser
(* parsers and parser builders for formal parameters and bindings ((higher-order)) S386b *)
fun asLambda inWhat (loc, e) =
  let
    fun loop (LAMBDA _) = true
      | loop (TYLAMBDA (_, e)) = loop e
      | loop _ = false
  in
    if loop e then
      OK e
      ...
      synerrorAt
        ("in " ^ inWhat ^ ", expression " ^ expString e ^ " is not a lambda")
        loc
  end
...
fun kw keyword = eqx keyword namelike
fun usageParsers ps =
  anyParser (map (usageParser kw) ps)
(* type declarations for consistency checking *)
...
fun atomicSchemeExpOf name =
  VAR <$> name <|> LITERAL <$> NUM <$> int <|> LITERAL <$> embedBool <$> booltok
(* parsers and parser builders for \scheme-like syntax S388a *)
fun fullSchemeExpOf atomic bracketedOf =
  let
    val exp = fn tokens => fullSchemeExpOf atomic bracketedOf tokens
  in
    atomic <|> bracketedOf exp <|> quote *> (LITERAL <$> sexp)
...
(* type declarations for consistency checking *)
val _ =
  op fullSchemeExpOf : exp parser -> (exp parser -> exp parser) -> exp parser
...
(* type declarations for consistency checking *)
val _ =
  op typedFormalsOf : string parser
                      -> 'b parser
                      -> 'a parser
                      -> string
...
fun arrowsOf conapp funty =
  let
    fun arrows [] [] = ERROR "empty type ()"
      | arrows (tycon :: tyargs) [] =
          OK (conapp (tycon, tyargs))
      | arrows args [rhs] =
          (case rhs of
             [result] => OK (funty (args, result))
           | [] => ERROR "no result type after function arrow"
           | _ => ERROR "multiple result types after function arrow")
      | arrows args (_ :: _ :: _) =
          ERROR "multiple arrows in function type"
  in
    fn xs => errorLabel "syntax error: " o arrows xs
  end
(* type declarations for consistency checking *)
...
(* type declarations for consistency checking *)
val _ =
  op distinctTBsIn : ((name * 't) * 'e) list parser
                     -> string
...
fun ty tokens =
  ...
  in
    ...
    <|>
    bracket
      ( "type application or function type"
...
val badColonParm =
  ...
  errorAtEnd
  (fn s =>
     [ "there must be a space between parameter "
     , s
...
val formal = bracket
  ("[x : ty]", badColonParm <|> pair <$> name <* kw ":" <*> ty)
...
fun tylambda a's exp =
  nodups ("formal type parameter", "type-lambda") a's
  >>=+ (fn a's => TYLAMBDA (a's, exp))

fun cb key usage parser =
  bracketKeyword (eqx key namelike, usage, parser)

fun exp tokens =
  (VAR <$> name <|> LITERAL <$> NUM <$> int <|> LITERAL <$> BOOLV <$> booltok
   <|> quote *> (LITERAL <$> sexp)
...
and letx kind bs exp =
  letDups kind bs >>=+ (fn bs => LETX (kind, bs, exp))
and tybindings ts =
  bindingsOf "([x : ty] e)" formal exp ts
and letrecbs ts =
  distinctTBsIn (bindingsOf "([x : ty] e)" formal (asLambda "letrec" exp))
    "letrec" ts
and bindings ts =
  bindingsOf "(x e)" name exp ts
...
val unit_test =
  ...
  <|>
  cb "check-expect" "(check-expect e1 e2)" (curry CHECK_EXPECT <$> exp <*> exp)
  <|> cb "check-type" "(check-type e tau)" (curry CHECK_TYPE <$> exp <*> ty)
  <|>
...
val xdef =
  DEF <$> def <|> cb "use" "(use filename)" (USE <$> name)
  <|> TEST <$> unit_test <|> badRight "unexpected right bracket"
  <|> DEF <$> EXP <$> exp <?> "definition"
(* type declarations for consistency checking *)
  ...
  in go
    ...
    let
      fun ev (LITERAL n) = n
        ...
        | ev (IFX (e1, e2, e3)) =
            ev (if projectBool (ev e1) then e2 else e3)
        | ev (WHILEX (guard, body)) =
            if projectBool (ev guard) then (ev body; ev (WHILEX (guard, body)))
            else unitVal
        | ev (BEGIN es) =
            let
              fun b (e :: es, lastval) =
        ...
        | ev (LAMBDA (args, body)) =
            CLOSURE ((map (fn (x, ty) => x) args, body), rho)
        (* more alternatives for [[ev]] for {\tuscheme} S412a *)
        | ev (APPLY (f, args)) =
            (case ev f of
               PRIMITIVE prim => prim (map ev args)
             | CLOSURE clo =>
                 (* apply closure [[clo]] to [[args]] ((mlscheme)) 310c *)
                 let
                   val ((formals, body), savedrho) = clo
                   val actuals = map ev args
                 in
                   eval (body, bindList (formals, map ref actuals, savedrho))
                   handle BindListLength =>
                     raise RuntimeError
                       ("Wrong number of arguments to closure; " ^ "expected ("
                        ^ spaceSep formals ^ ")")
                 end
        ...
        | ev (LETX (LETSTAR, bs, body)) =
            let
              fun step ((n, e), rho) =
                bind (n, ref (eval (e, rho)), rho)
            in
              eval (body, foldl step rho bs)
            end
        (* more alternatives for [[ev]] for {\tuscheme} S412c *)
        | ev (LETRECX (bs, body)) =
            let
              val (tynames, values) = ListPair.unzip bs
              val names = map fst tynames
              val rho' =
                rho <+> mkEnv (names, map (fn _ => ref (unspecified ())) values)
              val updates = map (fn ((x, _), e) => (x, eval (e, rho'))) bs
            in
              List.app (fn (x, v) => find (x, rho') := v) updates;
              eval (body, rho')
            end
      (* type declarations for consistency checking *)
...
fun processDef (d, (Delta, Gamma, rho), interactivity) =
  let
    ...
    val (Gamma, tystring) = typdef (d, Delta, Gamma)
    val (rho, valstring) = evaldef (d, rho)
    val _ =
      if echoes interactivity then println (valstring ^ " : " ^ tystring)
      else ()
    (* type declarations for consistency checking *)
...
fun dump_names (kinds, types, values) =
  app (println o fst) values (*OMIT*)
(* shared definition of [[withHandlers]] S239a *)
fun withHandlers f a caught =
  f a
  handle
    RuntimeError msg => caught ("Run-time error <at loc>: " ^ msg)
  ...
  | LeftAsExercise msg => caught ("left as exercise: " ^ msg)
...
fun testIsGood (test, (Delta, Gamma, rho)) =
  let
    ...
    fun checkExpectChecks (e1, e2) =
      ...
      in
        if eqType (tau1, tau2) then
          ...
          raise TypeError
            ("Expressions have types " ^ typeString tau1 ^ " and "
             ^ typeString tau2)
      end
      handle TypeError msg =>
        failtest
          ["In (check-expect ", expString e1, " ", expString e2, "), ", msg]
    ...
    fun checkTypeChecks (e, tau) =
      ...
      handle TypeError msg =>
        failtest
          ["In (check-type ", expString e, " " ^ typeString tau, "), ", msg]
    ...
    fun whatWasExpected (e, outcome) =
      case asSyntacticValue e of
        SOME v => valueString v
      | NONE =>
          case outcome of
            OK v => valueString v ^ " (from evaluating " ^ expString e ^ ")"
    ...
    fun checkExpectPassesWith equals (checkx, expectx) =
      case (outcome checkx, outcome expectx) of
        (OK check, OK expect) =>
          equals (check, expect)
          orelse
          failtest
    ...
    (* type declarations for consistency checking *)
    val _ =
      op checkExpectPassesWith : (value * value -> bool) -> exp * exp -> bool
    ...
    fun checkTypeErrorPasses (EXP e) =
          (let
           ...
           in
             failtest
               [ "check-type-error failed: expected "
               , expString e
               , " not to have a type, but it has type "
               , typeString tau
               ]
           end
           handle TypeError msg => true | Located (_, TypeError _) => true)
  ...
  in
    checks test andalso passes test
  end
...
and numberOfGoodTests (tests, rho) =
  let
    val testIsGood = fn args => (resetComputationLimits (); testIsGood args)
  in (*OMIT*)
...
fun readEvalPrintWith errmsg (xdefs, basis, interactivity) =
  let
    ...
    fun processXDef (xd, basis) =
      let (* definition of [[useFile]], to read from a file S238a *)
        fun useFile filename =
          ...
          in
            readEvalPrintWith errmsg
              (filexdefs (filename, fd, noPrompts), basis, inter')
            before TextIO.closeIn fd
          end
        fun try (USE filename) = useFile filename
          | try (TEST t) =
              (unitTests := t :: !unitTests; basis)
          | try (DEF def) = processDef (def, basis, interactivity)
          | try (DEFS ds) =
              foldl processXDef basis (map DEF ds)
        (*OMIT*)
        fun caught msg =
          (errmsg (stripAtLoc msg); basis)
      in
        withHandlers try xd caught
      end
    (* type declarations for consistency checking *)
    ...
    (* type declarations for consistency checking *)
    val _ =
      op readEvalPrintWith : (string -> unit)
                             -> xdef stream * basis * interactivity
                             -> basis
...
fun ternaryOp f =
  (fn [a, b, c] => f (a, b, c) | _ => raise BugInTypeChecking "arity 3")
fun binaryOp f =
  (fn [a, b] => f (a, b) | _ => raise BugInTypeChecking "arity 2")
fun unaryOp f =
  (fn [a] => f a | _ => raise BugInTypeChecking "arity 1")
(* type declarations for consistency checking *)
...
val primBasis =
  let
    ...
    fun addPrim ((name, prim, funty), (types, values)) =
      (bind (name, funty, types), bind (name, ref (PRIMITIVE prim), values))
    val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
      ((* primitive functions for \tuscheme\ [[::]] 381d *)
       ...
       ( "println"
       , unaryOp (fn x => (print (valueString x ^ "\n"); unitVal))
       , FORALL (["'a"], FUNTY ([tvA], unittype))
       )
       ::
       ( "print"
       , unaryOp (fn x => (print (valueString x); unitVal))
       , FORALL (["'a"], FUNTY ([tvA], unittype))
       )
       ::
       ( "printu"
       , unaryOp
           (fn NUM n => (printUTF8 n; unitVal)
       ...
       , FORALL (["'a"], FUNTY
           ([btreetype tvA, tvA, btreetype tvA], btreetype tvA))
       )
       ::
       ( "leaf?"
       , unaryOp (BOOLV o (fn NIL => true | _ => false))
       , FORALL (["'a"], FUNTY ([btreetype tvA], booltype))
       )
       ::
       ( "node?"
       , unaryOp (BOOLV o (fn NIL => false | _ => true))
       , FORALL (["'a"], FUNTY ([btreetype tvA], booltype))
       )
       ::
       ( "node-l"
       , unaryOp
           (fn PAIR (a, PAIR (b, c)) => a
             | NIL => raise (RuntimeError "node-l of leaf")
             | _ => raise (BugInTypeChecking "node-l of non-btree"))
       , FORALL (["'a"], FUNTY ([btreetype tvA], btreetype tvA))
       )
       ::
       ( "node-x"
       , unaryOp
           (fn PAIR (a, PAIR (b, c)) => b
             | NIL => raise (RuntimeError "node-x of leaf")
             | _ => raise (BugInTypeChecking "node-x of non-btree"))
       , FORALL (["'a"], FUNTY ([btreetype tvA], tvA))
       )
       ::
       ( "node-r"
       , unaryOp
           (fn PAIR (a, PAIR (b, c)) => c
             | NIL => raise (RuntimeError "node-r of leaf")
             | _ => raise (BugInTypeChecking "node-r of non-btree"))
       , FORALL (["'a"], FUNTY ([btreetype tvA], btreetype tvA))
       )
       ::
       [])
    fun addVal ((name, v, ty), (types, values)) =
      (bind (name, ty, types), bind (name, ref v, values))
    val (types, values) =
...
val initialBasis =
  let
    val xdefs = stringsxdefs ("predefined functions", predefs)
    fun predefinedFunctionError s = ()
  in
    readEvalPrintWith predefinedFunctionError (xdefs, primBasis, noninteractive)
  end
...
fun runStream inputName input interactivity basis =
  let
    val _ = setup_error_format interactivity
    val prompts = if prompts interactivity then stdPrompts else noPrompts
    val xdefs = filexdefs (inputName, input, prompts)
  in
    readEvalPrintWith eprintln (xdefs, basis, interactivity)
  end
(* type declarations for consistency checking *)
val _ = op withHandlers : ('a -> 'b) -> 'a -> (string -> 'b) -> 'b
(* type declarations for consistency checking *)
val _ =
  op runStream : string -> TextIO.instream -> interactivity -> basis -> basis
...
fun runPathWith interactivity ("-", basis) =
  ...
  | runPathWith interactivity (path, basis) =
      ...
      in runStream path fd interactivity basis before TextIO.closeIn fd
      end
(* type declarations for consistency checking *)
...
fun perform (RUN_WITH interactivity, []) =
  ...
  | perform (DUMP go, []) = go ()
  | perform (DUMP go, _ :: _) =
      perform (FAIL "Dump options take no files", [])
  | perform (FAIL msg, _) =
      (eprintln msg; !usage ())
...
fun merge (_, a as FAIL _) = a
  ...
  | merge (DEFAULT, a) = a
  | merge (_, DEFAULT) =
      raise InternalError "DEFAULT on the right in MERGE"
  | merge (RUN_WITH _, right as RUN_WITH _) = right
  | merge (DUMP f, DUMP g) =
      DUMP (g o f)
  | merge (_, r) =
      FAIL "Interpret or dump, but don't try to do both"
(* type declarations for consistency checking *)
...
val unusedActions = (* reveals answers to homeworks *) (*OMIT*)
  [("-predef", DUMP (fn () => app println predefs)) (*OMIT*)] (*OMIT*)
(* type declarations for consistency checking *)
...
fun action option =
  case List.find (curry op= option o fst) actions of
