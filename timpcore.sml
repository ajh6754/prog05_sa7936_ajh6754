...
fun typeString BOOLTY = "bool"
  ...
  | typeString (ARRAYTY tau) =
      "(array " ^ typeString tau ^ ")"
...
fun typeof (e, globals, functions, formals) =
  let (* function [[ty]], checks type of expression given $\itenvs$ 338b *)
    fun ty (LITERAL v) = INTTY
      ...
      | ty (VAR x) =
          (find (x, formals) handle NotFound _ => find (x, globals))
      ...
      | ty (IFX (e1, e2, e3)) =
          let
            ...
            val tau3 = ty e3
          in
            if eqType (tau1, BOOLTY) then
              if eqType (tau2, tau3) then
                tau2
              else
                raise TypeError
                  ("In if expression, true branch has type " ^ typeString tau2
      ...
      | ty (EQ (e1, e2)) =
          ...
          in
            if eqType (tau1, tau2) then
              ...
              raise TypeError
                ("Equality sees values of different types " ^ typeString tau1
                 ^ " and " ^ typeString tau2)
          end
      ...
      | ty (APPLY (f, actuals)) =
          let
            ...
            fun badParameter (n, atau :: actuals, ftau :: formals) =
                  if eqType (atau, ftau) then
                    ...
                    raise TypeError
                      ("In call to " ^ f ^ ", parameter " ^ intString n
                       ^ " has type " ^ typeString atau ^ " where type "
                       ^ typeString ftau ^ " is expected")
              | badParameter _ =
                  raise TypeError
                    ("Function " ^ f ^ " expects "
                     ^ countString formaltypes "parameter" ^ " but got "
                     ^ intString (length actualtypes))
            (* type declarations for consistency checking *)
          ...
          in
            ...
            else badParameter (1, actualtypes, formaltypes)
          end
...
fun typdef (d, globals, functions) =
  case d of (* cases for typing definitions in \timpcore 341c *)
    VAL (x, e) =>
      if not (isbound (x, globals)) then
        ...
        in
          if eqType (tau, tau') then
            ...
            raise TypeError
              ("Global variable " ^ x ^ " of type " ^ typeString tau'
  ...
  | DEFINE (f, {returns, formals, body}) =>
      ...
      in
        if eqType (tau, returns) then
          ...
          raise TypeError
            ("Body of function has type " ^ typeString tau
             ^ ", which does not match declared result type " ^ "of "
             ^ typeString returns)
      end
(* type declarations for consistency checking *)
...
fun impcorefun what exp =
  name <|> exp <!> ("only named functions can be " ^ what) <?> "function name"
(* parsers and [[xdef]] streams for \timpcore S400f *)
fun exp tokens =
  (atomicExp <|> exptable exp
...
fun repeatable_ty tokens =
  (BOOLTY <$ kw "bool" <|> UNITTY <$ kw "unit" <|> INTTY <$ kw "int"
   <|> badTypeName <$>! @@name <|> usageParsers [("(array ty)", ARRAYTY <$> ty)])
    tokens
and ty tokens =
  (repeatable_ty <?> "int, bool, unit, or (array ty)") tokens

val funty = bracket
  ("function type", curry FUNTY <$> many repeatable_ty <* arrow <*> ty)
(* type declarations for consistency checking *)
...
val testtable = usageParsers
  ...
  , ("(check-type-error d)", CHECK_TYPE_ERROR <$> (deftable <|> EXP <$> exp))
...
val xdeftable = usageParsers
  [("(use filename)", USE <$> name)(* rows added to \timpcore\ [[xdeftable]] in exercises S402c *) (* add syntactic extensions here, each preceded by a comma *) ]

val xdef =
  DEF <$> deftable <|> TEST <$> testtable <|> xdeftable
  <|> badRight "unexpected right bracket" <|> DEF <$> EXP <$> exp
  <?> "definition"
  ...
  in go
    ...
    let
      ...
      fun ev (LITERAL n) = n
        ...
        | ev (IFX (cond, t, f)) =
            if toBool (ev cond) then ev t else ev f
        | ev (WHILEX (cond, exp)) =
            if toBool (ev cond) then (ev exp; ev (WHILEX (cond, exp)))
            else unitVal
        | ev (BEGIN es) =
            let
              fun b (e :: es, lastval) =
        ...
        | ev (EQ (e1, e2)) =
            ofBool (eq (ev e1, ev e2))
        | ev (PRINTLN e) =
            (print (valueString (ev e) ^ "\n"); unitVal)
        | ev (PRINT e) =
            (print (valueString (ev e)); unitVal)
        | ev (APPLY (f, args)) =
            (case find (f, functions) of
               PRIMITIVE p => p (map ev args)
             | USERDEF func =>
                 (* apply user-defined function [[func]] to [[args]] S398a *)
                 let
                   val (formals, body) = func
                 ...
                 in
                   eval (body, globals, functions, mkEnv (formals, actuals))
        ...
        | ev (APUT (e1, e2, e3)) =
            ...
            in Array.update (toArray a, toInt i, v); v
            end
        | ev (AMAKE (len, init)) =
...
fun evaldef (d, globals, functions) =
  case d of
    VAL (x, e) => (* evaluate [[e]] and bind the result to [[x]] S398c *)
...
(* type declarations for consistency checking *)
val _ =
  op evaldef : def * value ref env * func env
               -> value ref env * func env * string
...
fun processDef (d, (tglobals, tfuns, vglobals, vfuns), interactivity) =
  let
    ...
    val (tglobals, tfuns, tystring) = typdef (d, tglobals, tfuns)
  ...
  in
    (tglobals, tfuns, vglobals, vfuns)
  end
fun dump_names (_, _, _, functions) =
  app (println o fst) functions (*OMIT*)
...
fun testIsGood (test, (tglobals, tfuns, vglobals, vfuns)) =
  let
    fun ty e =
      typeof (e, tglobals, tfuns, emptyEnv)
    ...
    fun checkFunctionTypePasses (f, tau as FUNTY (args, result)) =
      let
        val tau' as FUNTY (args', result') =
          find (f, tfuns)
          handle NotFound f =>
            raise TypeError ("Function " ^ f ^ " is not defined")
      in
        if eqTypes (args, args') andalso eqType (result, result') then
          true
        else
          failtest
            [ "check-function-type failed: expected "
            , f
            , " to have type "
            , funtyString tau
            , ", but it has type "
      ...
      handle TypeError msg =>
        failtest
          ["In (check-function-type ", f, " " ^ funtyString tau, "), ", msg]
...
val primitiveBasis: basis =
  let
    fun addPrim ((name, prim, funty), (tfuns, vfuns)) =
      (bind (name, funty, tfuns), bind (name, PRIMITIVE prim, vfuns))
    val (tfuns, vfuns) = foldl addPrim (emptyEnv, emptyEnv)
      ((* primitive functions for \timpcore\ [[::]] S393d *)
       ( "+"
       , arithOp op+
       , arithtype
       ) :: ("-", arithOp op-, arithtype) :: ("*", arithOp op*, arithtype)
...
val initialBasis =
  ...
  in
    readEvalPrintWith predefinedFunctionError
